...
const Tfood = () => {
  ...
  const [foodData, setFoodData] = useState(thfood);
  const searchFilter = (text) => {
    setSearchQuery(text);
    const filteredData = thfood.filter(item =>
  ...
  const calculateTotalCaloriesT = () => {
    ...
    // อัปเดตสถานะ barStatus
    const updatebarData = [
      { value: caloriesSum, label: 'Food', frontColor: 'red' },
      { value: 0, label: 'Drink', frontColor: '#177AD5' },
      { value: 0, label: 'Dessert', frontColor: 'pink' },
    ]
    setBarStatus(updatebarData); //นำไปใช้ต่อ ใน modal usestate (no.1)
    navigation.navigate('Cal', { totalCalories });
  };

  const navigateToCal = () => {
    navigation.navigate('Cal', { totalCalories: totalCalories }); // Navigate to Cal screen with totalCalories as a parameter
    setModalVisible(false); // Close the modal after navigating
  };
  

  //<Cal totalCaloriesT={totalCalories} /> // ส่งค่า totalCaloriesT ไปยังหน้า Cal
  ...
  return (
    <View style={{ gap: 15, margin: 20 }}>
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
        <Text style={{ fontSize: 20, fontWeight: 'bold', fontFamily: 'Mitr' }}>Calories in Thai food</Text>
        <TouchableOpacity onPress={calculateTotalCaloriesT} style={{ width: 100, height: 35, backgroundColor: '#11B3CF', borderRadius: 10, shadowOffset: { width: 0, height: 10 }, shadowOpacity: 0.25, shadowRadius: 3.5, elevation: 5, left: -5 }}>
        ...
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(0, 0, 0, 0.5)', }}>
          <View style={{ backgroundColor: 'white', padding: 10, justifyContent: 'center', borderRadius: 10, width: 365, height: 650 }}>
            <ScrollView>
              ...
              <Pressable onPress={() => navigateToCal()}>
          ...
          <View style={{ padding: 8 }}>
            <View style={{ width: 360, height: 75, backgroundColor: '#dedede', borderRadius: 10, shadowOffset: { width: 0, height: 10 }, shadowOpacity: 0.25, shadowRadius: 3.5, elevation: 5 }}>
              <View style={{ flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center', paddingHorizontal: 10 }}>
                <Image source={{ uri: item.foodimages || 'https://res.cloudinary.com/doukdigoy/image/upload/v1712826872/cpe451/Thai_Food_uhrw41.png' }} style={{ width: 60, height: 60, borderRadius: 30, top: 8 }} />
