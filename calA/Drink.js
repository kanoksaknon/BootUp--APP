...
const Drink = () => {
  ...
  const [foodData, setFoodData] = useState(Mdrink);
  const searchFilter = (text) => {
    setSearchQuery(text);
    const filteredData = Mdrink.filter(item =>
  ...
  const calculateTotalCaloriesD = () => {
    ...
    const updatebarData = [
      ...
      { value: caloriesSum, label: 'Drink', frontColor: '#177AD5' },
  ...
  return (
    <View style={{gap: 15, margin: 20}}>
      ...
      <View style={{flexDirection: 'row', justifyContent: 'space-between'}}>
        <Text style={{fontSize: 20, fontWeight: 'bold',fontFamily: 'Mitr'}}>Calories in Drink</Text>
        <TouchableOpacity onPress={calculateTotalCaloriesD} style={{width: 100, height: 35, backgroundColor: '#11B3CF', borderRadius: 10, shadowOffset: { width: 0, height: 10 }, shadowOpacity: 0.25, shadowRadius: 3.5, elevation: 5, left: -5}}>
